                ...
                {wishlist.location.time && (
                  <div className="flex gap-4 items-center">
                    ...
                    <div className="text-xl font-medium text-foreground">
                      {new Date(wishlist.location.time).toLocaleDateString('ru-RU', { day: 'numeric', month: 'long', year: 'numeric' })}
                    </div>
                  </div>
                )}
          ...
          <div className="space-y-12">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {presents?.map(present => (
                <PresentItem key={present.id} present={present} theme={wishlist.settings.colorScheme} isHidden={isPresentHidden} wishlistId={wishlist.id} />
              ))}
            </div>
          </div>
        </div>
